...
class CatUniScene(Scene):
    ...
    def render(self):
        ...
        #draw cat
        pygame.draw.line(screen, [0,0,255], self.cat_location, self.cat_head_location, 20)
        ...
        #draw score
        textsurface = self.myfont.render("score : "+ str(self.score), True, [255,255,255])
    ...
    def tick(self, dt):
        ...
        if self.right_pressed:
            self.cat_speed[0] = min(self.cat_speed[0] + 0.3*time_elapsed/17, self.cat_speed_max)
            self.cat_angle -= 0.01*time_elapsed/17

        if self.left_pressed:
            self.cat_speed[0] = max(self.cat_speed[0] - 0.3*time_elapsed/17, -self.cat_speed_max)
            self.cat_angle += 0.01*time_elapsed/17
        ...
        if self.cat_location[1] > height - 100:
            ...
            self.cat_speed[1] = 0
        self.cat_head_location = [int(self.cat_location[0] + 100*math.cos(self.cat_angle - math.pi/2)), int(self.cat_location[1] + 100*math.sin(self.cat_angle - math.pi/2))]
        ...
        for f in reversed(self.not_fish):
            if distance([f[0], f[1]], self.cat_head_location) < 50:
                self.not_fish.remove(f)
                self.angle_to_not_fish = math.atan2(self.cat_head_location[1] - f[1], self.cat_head_location[0] - f[0]) - math.pi/2
        ...
        while len(self.fish) < 1:
            ...
            if random.choice([0,1]) == 0:
                self.fish.append([0,random.randint(0, height/2), random.randint(2,5), -random.randint(3,10)])
            else:
                self.fish.append([width,random.randint(0, height/2), -random.randint(2,5), -random.randint(3,10)])
        while len(self.not_fish) < 1:
            #choose a side of the screen
            if random.choice([0,1]) == 0:
                self.not_fish.append([0,random.randint(0, height/2), random.randint(2,5), -random.randint(3,10)])
            else:
                self.not_fish.append([width,random.randint(0, height/2), -random.randint(2,5), -random.randint(3,10)])
